...
module.exports = (a,method) => {
    ...
    if(method === 'count'){
        result = `(${num_letters(a[0]).trimLeft()})` + declOfNum(a[0], 'запро', ['с', 'са', 'сов'])
    } else if (method === 'digit'){
        result = `(${num_letters(a[0]).trimLeft()})`
    }else{
        result = `(${num_letters(a[0]).trimLeft()})` + declOfNum(a[0], 'рубл', ['ь', 'я', 'ей']) + ' ' + a[1] + declOfNum(a[1], 'копе', ['йка', 'йки', 'ек'])
    }
    return result
